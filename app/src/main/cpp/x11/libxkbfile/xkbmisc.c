...
Bool
XkbWriteXKBKeymapForNames2(FILE *                file,
...
int
XkbFindKeycodeByName2(XkbDescPtr xkb, char *name, Bool use_aliases)
{
    register int i;
...
unsigned
XkbConvertGetByNameComponents2(Bool toXkm, unsigned orig)
{
    unsigned rtrn;
...
Bool
XkbNameMatchesPattern2(char *name, char *ptrn)
{
    while (ptrn[0] != '\0') {
        if (name[0] == '\0') {
            if (ptrn[0] == '*') {
        ...
        if (ptrn[0] == '?') {
            ...
            if ((!UNMATCHABLE(name[0])) &&
                XkbNameMatchesPattern2(name + 1, ptrn))
                return True;
            return XkbNameMatchesPattern2(name, ptrn + 1);
        }
