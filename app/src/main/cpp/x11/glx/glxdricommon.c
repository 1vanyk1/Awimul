...
static const char dri_driver_path[] = "/usr/X11/lib/dri";
               ...
               void **renderExt, const char *renderName, int renderVersion)
{
    ...
    if (*renderExt == NULL) {
        LogMessage(X_ERROR,
                   "AIGLX error: %s does not export required DRI extension (renderExt)\n",
                   driverName);
        goto cleanup_failure;
    }
    ...
    if (*coreExt == NULL) {
        LogMessage(X_ERROR,
                   "AIGLX error: %s does not export required DRI extension (coreExt)\n",
                   driverName);
        goto cleanup_failure;
    }
    return driver;
