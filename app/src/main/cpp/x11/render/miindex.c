...
miBuildRenderColormap(ColormapPtr pColormap, Pixel * pixels, int *nump)
{
    ...
    if (pColormap->mid != pColormap->pScreen->defColormap) {
        ...
        if (policy == PictureCmapPolicyDefault) {
            if (avail >= 256 &&
                (pColormap->pVisual->_class | DynamicClass) == PseudoColor)
    ...
    switch (policy) {
        case PictureCmapPolicyAll:
             ...
             */
            if ((pColormap->pVisual->_class | DynamicClass) == PseudoColor) {
                for (cube = 1;
...
miInitIndexed(ScreenPtr pScreen, PictFormatPtr pFormat)
{
    ...
    if (pVisual->_class & DynamicClass) {
     ...
     * done a bit quicker...
     */
    switch (pVisual->_class | DynamicClass) {
        case GrayScale:
