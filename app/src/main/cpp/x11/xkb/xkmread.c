...
Atom
XkbInternAtom2(char *str, Bool only_if_exists)
{
    if (str == NULL)
        return None;
    ...
    for (i = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1) {
        ...
        if (named & bit) {
            if (nRead += XkmGetCountedString(file, name, 100)) {
                xkb->names->vmods[i] = XkbInternAtom2(name, FALSE);
                if (changes)
...
#define WANTED (XkbKeycodesNameMask|XkbKeyNamesMask|XkbKeyAliasesMask)
    ...
    if (name[0] != '\0') {
        xkb->names->keycodes = XkbInternAtom2(name, FALSE);
    }
...
ReadXkmKeyTypes(FILE * file, XkbDescPtr xkb, XkbChangesPtr changes)
{
    ...
    if (buf[0] != '\0') {
        ...
        xkb->names->types = XkbInternAtom2(buf, FALSE);
    }
    ...
    for (i = 0; i < num_types; i++, type++) {
        ...
        if (buf[0] != '\0') {
            type->name = XkbInternAtom2(buf, FALSE);
        }
        else
            type->name = None;
            ...
            if (type->level_names != NULL) {
                for (n = 0; n < wire.nLevelNames; n++) {
                    ...
                    if (strlen(buf) == 0)
                        ...
                        type->level_names[n] = XkbInternAtom2(buf, 0);
                }
            }
        }
...
ReadXkmCompatMap(FILE * file, XkbDescPtr xkb, XkbChangesPtr changes)
{
    ...
    if (name[0] != '\0') {
        ...
        xkb->names->compat = XkbInternAtom2(name, FALSE);
    }
        ...
        if (wireMap.flags & XkmKeyHasTypes) {
            for (g = 0; g < XkbNumKbdGroups; g++) {
                if ((wireMap.flags & (1 << g)) &&
                    ...
                    typeName[g] = XkbInternAtom2(buf, 1);
                    nRead += tmp;
                }
...
ReadXkmGeomDoodad(FILE * file, XkbGeometryPtr geom, XkbSectionPtr section)
{
    ...
    doodad = XkbAddGeomDoodad(geom, section, XkbInternAtom2(buf, FALSE));
    if (!doodad)
        return nRead;
...
ReadXkmGeomOverlay(FILE * file, XkbGeometryPtr geom, XkbSectionPtr section)
{
    ...
    ol = XkbAddGeomOverlay(section, XkbInternAtom2(buf, FALSE), olWire.num_rows);
    if (!ol)
        return nRead;
...
ReadXkmGeomSection(FILE * file, XkbGeometryPtr geom)
{
    ...
    nRead += XkmGetCountedString(file, buf, 100);
    nameAtom = XkbInternAtom2(buf, FALSE);
...
ReadXkmGeometry(FILE * file, XkbDescPtr xkb)
{
    ...
    geom = xkb->geom;
    geom->name = XkbInternAtom2(buf, FALSE);
...
#define	MAX_TOC	16
unsigned
XkmReadFile2(FILE * file, unsigned need, unsigned want, XkbDescPtr *xkb)
{
    register unsigned i;
