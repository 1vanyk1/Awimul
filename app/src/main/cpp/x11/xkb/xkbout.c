...
{
    fprintf(file, "%s", XkbActionText2(xkb, (XkbAction *) action, XkbXKBFile));
    return TRUE;
}
...
Bool
XkbWriteXKBKeycodes2(FILE * file,
                    XkbDescPtr xkb,
                    Bool topLevel,
                    Bool showImplicit, XkbFileAddOnFunc2 addOn, void *priv)
{
    Atom kcName;
    register unsigned i;
    ...
    if (kcName != None)
        fprintf(file, "xkb_keycodes \"%s\" {\n",
                XkbAtomText2(kcName, XkbXKBFile));
    else
    ...
    for (i = xkb->min_key_code; i <= xkb->max_key_code; i++) {
        if (xkb->names->keys[i].name[0] != '\0') {
            if (XkbFindKeycodeByName(xkb, xkb->names->keys[i].name, TRUE) != i)
                ...
                alternate = "";
            fprintf(file, "    %s%6s = %d;\n", alternate,
                    XkbKeyNameText2(xkb->names->keys[i].name, XkbXKBFile), i);
        }
    }
        ...
        for (i = 0; i < XkbNumIndicators; i++) {
            ...
            if (xkb->names->indicators[i] != None) {
                fprintf(file, "%sindicator %d = \"%s\";\n", type, i + 1,
                        XkbAtomText2(xkb->names->indicators[i], XkbXKBFile));
            }
        }
    }
    if (xkb->names->key_aliases != NULL) {
        ...
        for (i = 0; i < xkb->names->num_key_aliases; i++, pAl++) {
            fprintf(file, "    alias %6s = %6s;\n",
                    XkbKeyNameText2(pAl->alias, XkbXKBFile),
                    XkbKeyNameText2(pAl->real, XkbXKBFile));
        }
    }
    if (addOn)
...
Bool
XkbWriteXKBKeyTypes2(FILE * file,
                    XkbDescPtr xkb,
                    Bool topLevel,
                    Bool showImplicit, XkbFileAddOnFunc2 addOn, void *priv)
{
    ...
    if ((xkb->names == NULL) || (xkb->names->types == None))
        ...
        fprintf(file, "xkb_types \"%s\" {\n\n",
                XkbAtomText2(xkb->names->types, XkbXKBFile));
...
WriteXKBIndicatorMap(FILE * file,
                     XkbDescPtr xkb,
                     Atom name,
                     XkbIndicatorMapPtr led, XkbFileAddOnFunc2 addOn, void *priv)
{
    ...
    if (led->which_groups != 0) {
        if (led->which_groups != XkbIM_UseEffective) {
            fprintf(file, "        whichGroupState= %s;\n",
                    XkbIMWhichStateMaskText2(led->which_groups, XkbXKBFile));
        }
    ...
    if (led->which_mods != 0) {
        if (led->which_mods != XkbIM_UseEffective) {
            fprintf(file, "        whichModState= %s;\n",
                    XkbIMWhichStateMaskText2(led->which_mods, XkbXKBFile));
        }
        fprintf(file, "        modifiers= %s;\n",
                XkbVModMaskText2(xkb,
    ...
    if (led->ctrls != 0) {
        fprintf(file, "        controls= %s;\n",
                XkbControlsMaskText2(led->ctrls, XkbXKBFile));
    }
    if (addOn)
...
Bool
XkbWriteXKBCompatMap2(FILE * file,
                     XkbDescPtr xkb,
                     Bool topLevel,
                     Bool showImplicit, XkbFileAddOnFunc2 addOn, void *priv)
{
    ...
    if ((xkb->names == NULL) || (xkb->names->compat == None))
        ...
        fprintf(file, "xkb_compatibility \"%s\" {\n\n",
                XkbAtomText2(xkb->names->compat, XkbXKBFile));
    WriteXKBVModDecl(file, xkb,
                     (showImplicit ? VMOD_COMMENT_VALUE : VMOD_HIDE_VALUE));
    ...
    for (i = 0; i < xkb->compat->num_si; i++, interp++) {
        fprintf(file, "    interpret %s+%s(%s) {\n",
                ((interp->sym == NoSymbol) ? "Any" :
                 XkbKeysymText2(interp->sym, XkbXKBFile)),
                XkbSIMatchText2(interp->match, XkbXKBFile),
                XkbModMaskText2(interp->mods, XkbXKBFile));
        if (interp->virtual_mod != XkbNoModifier) {
            fprintf(file, "        virtualModifier= %s;\n",
                    XkbVModIndexText2(xkb, interp->virtual_mod, XkbXKBFile));
        }
                   ...
                   Bool showImplicit, XkbFileAddOnFunc2 addOn, void *priv)
{
    ...
    if ((xkb->names == NULL) || (xkb->names->symbols == None))
        ...
        fprintf(file, "xkb_symbols \"%s\" {\n\n",
                XkbAtomText2(xkb->names->symbols, XkbXKBFile));
    for (tmp = i = 0; i < XkbNumKbdGroups; i++) {
        if (xkb->names->groups[i] != None) {
            fprintf(file, "    name[group%d]=\"%s\";\n", i + 1,
                    XkbAtomText2(xkb->names->groups[i], XkbXKBFile));
            tmp++;
        }
    }
    if (tmp > 0)
    ...
    for (i = xkb->min_key_code; i <= xkb->max_key_code; i++) {
        ...
        simple = TRUE;
        fprintf(file, "    key %6s {",
                XkbKeyNameText2(xkb->names->keys[i].name, XkbXKBFile));
        if (srv->explicit) {
                ...
                if (multi) {
                    for (g = 0; g < XkbKeyNumGroups(xkb, i); g++) {
                        ...
                        if (srv->explicit[i] & (1 << g)) {
                            fprintf(file, "\n%s      type[group%d]= \"%s\",",
                                    comment, g + 1,
                                    XkbAtomText2(map->types[typeNdx].name,
                                                XkbXKBFile));
                        }
                        else if (showImplicit) {
                            fprintf(file, "\n//      type[group%d]= \"%s\",",
                                    g + 1, XkbAtomText2(map->types[typeNdx].name,
                                                       XkbXKBFile));
                        }
                    }
                }
                    ...
                    fprintf(file, "\n%s      type= \"%s\",", comment,
                            XkbAtomText2(map->types[typeNdx].name, XkbXKBFile));
                }
                simple = FALSE;
            }
        ...
        if (srv->behaviors != NULL) {
            ...
            if (type != XkbKB_Default) {
                simple = FALSE;
                fprintf(file, "\n        %s,",
                        XkbBehaviorText2(xkb, &srv->behaviors[i], XkbXKBFile));
            }
        }
        ...
        if (simple) {
            ...
            for (s = 0; s < XkbKeyGroupWidth(xkb, i, XkbGroup1Index); s++) {
                if (s != 0)
                    fprintf(file, ", ");
                fprintf(file, "%15s", XkbKeysymText2(*syms++, XkbXKBFile));
            }
            fprintf(file, " ] };\n");
        }
        else {
            ...
            for (g = 0; g < XkbKeyNumGroups(xkb, i); g++) {
                ...
                for (s = 0; s < XkbKeyGroupWidth(xkb, i, g); s++) {
                    if (s != 0)
                        fprintf(file, ", ");
                    fprintf(file, "%15s", XkbKeysymText2(syms[s], XkbXKBFile));
                }
                fprintf(file, " ]");
                ...
                for (bit = 1, n = 0; n < XkbNumModifiers; n++, bit <<= 1) {
                    if (map->modmap[i] & bit) {
                        ...
                        fprintf(file, "    modifier_map %s { <%s> };\n",
                                XkbModIndexText2(n, XkbXKBFile), buf);
                    }
                }
            }
        }
                ...
                XkbOutlinePtr outline, int lastRadius, int first, int indent)
{
    ...
    fprintf(file, "%s", iStr = XkbIndentText2(first));
    if (first != indent)
        iStr = XkbIndentText2(indent);
    if (outline->corner_radius != lastRadius) {
        fprintf(file, "corner= %s,",
                XkbGeomFPText2(outline->corner_radius, XkbMessage));
        if (shape != NULL) {
            fprintf(file, "\n%s", iStr);
        }
    }
    ...
    for (pt = outline->points, i = 0; i < outline->num_points; i++, pt++) {
        if (i == 0)
            ...
            fprintf(file, ", ");
        fprintf(file, "[ %3s, %3s ]", XkbGeomFPText2(pt->x, XkbXKBFile),
                XkbGeomFPText2(pt->y, XkbXKBFile));
    }
    fprintf(file, " }");
    return TRUE;
}
               ...
               unsigned indent, XkbGeometryPtr geom, XkbDoodadPtr doodad)
{
    ...
    i_str = XkbIndentText2(indent);
    fprintf(file, "%s%s \"%s\" {\n", i_str,
            XkbDoodadTypeText2(doodad->any.type, XkbMessage),
            XkbAtomText2(doodad->any.name, XkbMessage));
    fprintf(file, "%s    top=      %s;\n", i_str,
            XkbGeomFPText2(doodad->any.top, XkbXKBFile));
    fprintf(file, "%s    left=     %s;\n", i_str,
            XkbGeomFPText2(doodad->any.left, XkbXKBFile));
    ...
    switch (doodad->any.type) {
        ...
        case XkbSolidDoodad:
            if (doodad->shape.angle != 0) {
                fprintf(file, "%s    angle=  %s;\n", i_str,
                        XkbGeomFPText2(doodad->shape.angle, XkbXKBFile));
            }
            ...
            fprintf(file, "%s    shape= \"%s\";\n", i_str,
                    XkbAtomText2(shape->name, XkbXKBFile));
            break;
        case XkbTextDoodad:
            if (doodad->text.angle != 0) {
                fprintf(file, "%s    angle=  %s;\n", i_str,
                        XkbGeomFPText2(doodad->text.angle, XkbXKBFile));
            }
            if (doodad->text.width != 0) {
                fprintf(file, "%s    width=  %s;\n", i_str,
                        XkbGeomFPText2(doodad->text.width, XkbXKBFile));

            }
            if (doodad->text.height != 0) {
                fprintf(file, "%s    height=  %s;\n", i_str,
                        XkbGeomFPText2(doodad->text.height, XkbXKBFile));

            }
            if (doodad->text.color_ndx != 0) {
                color = XkbTextDoodadColor(geom, &doodad->text);
                fprintf(file, "%s    color= \"%s\";\n", i_str,
                        XkbStringText2(color->spec, XkbXKBFile));
            }
            fprintf(file, "%s    XFont= \"%s\";\n", i_str,
                    XkbStringText2(doodad->text.font, XkbXKBFile));
            fprintf(file, "%s    text=  \"%s\";\n", i_str,
                    XkbStringText2(doodad->text.text, XkbXKBFile));
            break;
        ...
        case XkbLogoDoodad:
            fprintf(file, "%s    logoName= \"%s\";\n", i_str,
                    XkbStringText2(doodad->logo.logo_name, XkbXKBFile));
            if (doodad->shape.angle != 0) {
                fprintf(file, "%s    angle=  %s;\n", i_str,
                        XkbGeomFPText2(doodad->logo.angle, XkbXKBFile));
            }
            if (doodad->shape.color_ndx != 0) {
                fprintf(file, "%s    color= \"%s\";\n", i_str,
                ...
                unsigned indent, XkbGeometryPtr geom, XkbOverlayPtr ol)
{
    ...
    if (ol->name != None) {
        fprintf(file, "%soverlay \"%s\" {\n", i_str,
                XkbAtomText2(ol->name, XkbMessage));
    }
    else
    ...
    for (nOut = r = 0, row = ol->rows; r < ol->num_rows; r++, row++) {
        for (k = 0, key = row->keys; k < row->num_keys; k++, key++) {
            ...
            over = XkbKeyNameText2(key->over.name, XkbXKBFile);
            under = XkbKeyNameText2(key->under.name, XkbXKBFile);
            if (nOut == 0)
...
WriteXKBSection(FILE * file, XkbSectionPtr s, XkbGeometryPtr geom)
{
    ...
    fprintf(file, "    section \"%s\" {\n", XkbAtomText2(s->name, XkbXKBFile));
    if (s->rows && (s->rows->num_keys > 0)) {
        dfltKeyColor = s->rows->keys[0].color_ndx;
        fprintf(file, "        key.color= \"%s\";\n",
                XkbStringText2(geom->colors[dfltKeyColor].spec, XkbXKBFile));
    }
    fprintf(file, "        priority=  %d;\n", s->priority);
    fprintf(file, "        top=       %s;\n",
            XkbGeomFPText2(s->top, XkbXKBFile));
    fprintf(file, "        left=      %s;\n",
            XkbGeomFPText2(s->left, XkbXKBFile));
    fprintf(file, "        width=     %s;\n",
            XkbGeomFPText2(s->width, XkbXKBFile));
    fprintf(file, "        height=    %s;\n",
            XkbGeomFPText2(s->height, XkbXKBFile));
    if (s->angle != 0) {
        fprintf(file, "        angle=  %s;\n",
                XkbGeomFPText2(s->angle, XkbXKBFile));
    }
    for (i = 0, row = s->rows; i < s->num_rows; i++, row++) {
        fprintf(file, "        row {\n");
        fprintf(file, "            top=  %s;\n",
                XkbGeomFPText2(row->top, XkbXKBFile));
        fprintf(file, "            left= %s;\n",
                XkbGeomFPText2(row->left, XkbXKBFile));
        if (row->vertical)
        ...
        if (row->num_keys > 0) {
            ...
            for (k = 0, key = row->keys; k < row->num_keys; k++, key++) {
                ...
                fprintf(file, "{ %6s, \"%s\", %3s",
                        XkbKeyNameText2(key->name.name, XkbXKBFile),
                        XkbAtomText2(shape->name, XkbXKBFile),
                        XkbGeomFPText2(key->gap, XkbXKBFile));
                if (key->color_ndx != dfltKeyColor) {
...
Bool
XkbWriteXKBGeometry2(FILE * file,
                    XkbDescPtr xkb,
                    Bool topLevel,
                    Bool showImplicit, XkbFileAddOnFunc2 addOn, void *priv)
{
    ...
    if (geom->name == None)
        ...
        fprintf(file, "xkb_geometry \"%s\" {\n\n",
                XkbAtomText2(geom->name, XkbXKBFile));
    fprintf(file, "    width=       %s;\n",
            XkbGeomFPText2(geom->width_mm, XkbXKBFile));
    fprintf(file, "    height=      %s;\n\n",
            XkbGeomFPText2(geom->height_mm, XkbXKBFile));
    ...
    if (geom->base_color != NULL)
        fprintf(file, "    baseColor=   \"%s\";\n",
                XkbStringText2(geom->base_color->spec, XkbXKBFile));
    if (geom->label_color != NULL)
        fprintf(file, "    labelColor=  \"%s\";\n",
                XkbStringText2(geom->label_color->spec, XkbXKBFile));
    if (geom->label_font != NULL)
        fprintf(file, "    xfont=       \"%s\";\n",
                XkbStringText2(geom->label_font, XkbXKBFile));
    ...
    if (geom->num_properties > 0) {
        ...
        for (prop = geom->properties, i = 0; i < geom->num_properties;
             i++, prop++) {
            fprintf(file, "    %s= \"%s\";\n", prop->name,
                    XkbStringText2(prop->value, XkbXKBFile));
        }
        fprintf(file, "\n");
    }
