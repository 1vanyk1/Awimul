...
char *
XkbAtomText2(Atom atm, unsigned format)
{
    const char *atmstr;
    char *rtrn, *tmp;
    ...
    return XkbStringText2(rtrn, format);
}

/***====================================================================***/

char *
XkbVModIndexText2(XkbDescPtr xkb, unsigned ndx, unsigned format)
{
    register int len;
...
char *
XkbVModMaskText2(XkbDescPtr xkb,
                unsigned modMask, unsigned mask, unsigned format)
{
    ...
    if (modMask != 0)
        mm = XkbModMaskText2(modMask, format);
    else
        mm = NULL;
    ...
    if (mask) {
        ...
        for (i = 0, bit = 1; i < XkbNumVirtualMods; i++, bit <<= 1) {
            if (mask & bit) {
                tmp = XkbVModIndexText2(xkb, i, format);
...
char *
XkbModIndexText2(unsigned ndx, unsigned format)
{
    char *rtrn;
    char buf[100];
...
char *
XkbModMaskText2(unsigned mask, unsigned format)
{
    register int i, bit;
    char buf[64], *rtrn;
...
/*ARGSUSED*/ char *
XkbConfigText2(unsigned config, unsigned format)
{
    static char *buf;
...
char *
XkbKeysymText2(KeySym sym, unsigned format)
{
    static char buf[32];
...
char *
XkbKeyNameText2(char *name, unsigned format)
{
    char *buf;
...
const char *
XkbSIMatchText2(unsigned type, unsigned format)
{
    static char buf[40];
    const char *rtrn;
...
char *
XkbIMWhichStateMaskText2(unsigned use_which, unsigned format)
{
    int len;
    unsigned i, bit, tmp;
    char *buf;
...
char *
XkbControlsMaskText2(unsigned ctrls, unsigned format)
{
    int len;
    unsigned i, bit, tmp;
    char *buf;
...
char *
XkbStringText2(char *str, unsigned format)
{
    char *buf;
...
char *
XkbGeomFPText2(int val, unsigned format)
{
    int whole, frac;
    char *buf;
...
char *
XkbDoodadTypeText2(unsigned type, unsigned format)
{
    char *buf;
...
const char *
XkbActionTypeText2(unsigned type, unsigned format)
{
    static char buf[32];
    const char *rtrn;
...
CopyModActionArgs(XkbDescPtr xkb, XkbAction *action, char *buf, int *sz)
{
    ...
    if (act->flags & XkbSA_UseModMapMods)
        ...
        TryCopyStr(buf,
                   XkbVModMaskText2(xkb, act->real_mods, tmp, XkbXKBFile), sz);
    }
    else
...
CopyISOLockArgs(XkbDescPtr xkb, XkbAction *action, char *buf, int *sz)
{
    ...
    if (act->flags & XkbSA_ISODfltIsGroup) {
        ...
        if (act->flags & XkbSA_UseModMapMods)
            ...
            if (act->real_mods) {
                TryCopyStr(buf, XkbModMaskText2(act->real_mods, XkbXKBFile), sz);
                if (tmp)
                    TryCopyStr(buf, "+", sz);
            }
            if (tmp)
                TryCopyStr(buf, XkbVModMaskText2(xkb, 0, tmp, XkbXKBFile), sz);
        }
        else
            TryCopyStr(buf, "none", sz);
    }
...
CopyRedirectKeyArgs(XkbDescPtr xkb, XkbAction *action, char *buf, int *sz)
{
    ...
    if (xkb && xkb->names && xkb->names->keys && (kc <= xkb->max_key_code) &&
        ...
        kn = XkbKeyNameText2(xkb->names->keys[kc].name, XkbXKBFile);
    ...
    if ((act->mods_mask == XkbAllModifiersMask) &&
        ...
        tmp = XkbVModMaskText2(xkb, act->mods, vmods, XkbXKBFile);
        TryCopyStr(buf, ",mods=", sz);
        TryCopyStr(buf, tmp, sz);
    }
    else {
        if ((act->mods_mask & act->mods) || (vmods_mask & vmods)) {
            tmp = XkbVModMaskText2(xkb, act->mods_mask & act->mods,
...
char *
XkbActionText2(XkbDescPtr xkb, XkbAction *action, unsigned format)
{
    ...
    if (format == XkbCFile) {
        snprintf(buf, sizeof(buf),
                 ...
                 XkbActionTypeText2(action->type, XkbCFile),
        ...
        snprintf(buf, sizeof(buf), "%s(",
                 XkbActionTypeText2(action->type, XkbXKBFile));
...
char *
XkbBehaviorText2(XkbDescPtr xkb, XkbBehavior * behavior, unsigned format)
{
    char buf[256], *tmp;
