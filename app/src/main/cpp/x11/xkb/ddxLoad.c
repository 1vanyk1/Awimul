...
#include "../headers/extensions/XKM.h"
...
extern main_xkbcomp(int argc, char *argv[]);
...
//                 "cat dev/null") == -1)
    ...
    argv[1] = "-w";
    char *arg1;
    asprintf(&arg1, "%d", ((xkbDebugFlags < 2) ? 1 : ((xkbDebugFlags > 10) ? 10 : (int) xkbDebugFlags)));
    argv[2] = arg1;
    argv[3] = xkbbasedirflag ? xkbbasedirflag : "";
    argv[4] = "-xkm";
    char *arg5;
    asprintf(&arg5, "%sstdin", xkm_output_dir);
    argv[5] = arg5;
    argv[6] = "-em1";
    argv[7] = PRE_ERROR_MSG;
    argv[8] = "-emp";
    argv[9] = ERROR_PREFIX;
    argv[10] = "-eml";
    argv[11] = POST_ERROR_MSG1;
    char *arg12;
    asprintf(&arg12, "%s%s.xkm", xkm_output_dir, keymap);
    argv[12] = arg12;
...
//    out = Popen(buf, "w");
    ...
    if (out != NULL) {
        ...
        int ooout = fclose(out);
        main_xkbcomp(argc, argv);
...
LoadXKM(unsigned want, unsigned need, const char *keymap, XkbDescPtr *xkbRtrn)
{
    ...
    missing = XkmReadFile2(file, need, want, xkbRtrn);
    if (*xkbRtrn == NULL) {
