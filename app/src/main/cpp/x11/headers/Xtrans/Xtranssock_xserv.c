...
#ifdef TCPCONN
static int TRANS_XSERV(SocketINETClose) (XtransConnInfo ciptr);
#endif
...
static int
TRANS_XSERV(SocketSelectFamily) (int first, const char *family)
...
static int
TRANS_XSERV(SocketINETGetAddr) (XtransConnInfo ciptr)
...
static int
TRANS_XSERV(SocketINETGetPeerAddr) (XtransConnInfo ciptr)
...
TRANS_XSERV(SocketOpen) (int i, int type)
...
static XtransConnInfo
TRANS_XSERV(SocketReopen) (int i _X_UNUSED, int type, int fd, const char *port)
...
static XtransConnInfo
TRANS_XSERV(SocketOpenCOTSClientBase) (const char *transname, const char *protocol,
                                 const char *host, const char *port, int previndex)
{
    ...
    while ((i = TRANS_XSERV(SocketSelectFamily) (i, transname)) >= 0) {
        if ((ciptr = TRANS_XSERV(SocketOpen) (
...
static XtransConnInfo
TRANS_XSERV(SocketOpenCOTSClient) (Xtransport *thistrans, const char *protocol,
                             const char *host, const char *port)
{
    return TRANS_XSERV(SocketOpenCOTSClientBase)(
...
static XtransConnInfo
TRANS_XSERV(SocketOpenCOTSServer) (Xtransport *thistrans, const char *protocol,
			     const char *host, const char *port)

{
    ...
    while ((i = TRANS_XSERV(SocketSelectFamily) (i, thistrans->TransName)) >= 0) {
	if ((ciptr = TRANS_XSERV(SocketOpen) (
		 i, Sockettrans2devtab[i].devcotsname)) != NULL)
	    break;
    }
...
static XtransConnInfo
TRANS_XSERV(SocketReopenCOTSServer) (Xtransport *thistrans, int fd, const char *port)

{
    ...
    while ((i = TRANS_XSERV(SocketSelectFamily) (i, thistrans->TransName)) >= 0) {
	if ((ciptr = TRANS_XSERV(SocketReopen) (
...
static int
TRANS_XSERV(SocketSetOption) (XtransConnInfo ciptr, int option, int arg)
...
static int
TRANS_XSERV(SocketCreateListener) (XtransConnInfo ciptr,
...
#ifdef TCPCONN
static int
TRANS_XSERV(SocketINETCreateListener) (XtransConnInfo ciptr, const char *port,
                                 unsigned int flags)

{
    ...
    if ((status = TRANS_XSERV(SocketCreateListener) (ciptr,
    ...
    if (TRANS_XSERV(SocketINETGetAddr) (ciptr) < 0)
    {
	prmsg (1,
...
static int
TRANS_XSERV(SocketUNIXCreateListener) (XtransConnInfo ciptr, const char *port,
				 unsigned int flags)

{
    ...
    if ((status = TRANS_XSERV(SocketCreateListener) (ciptr, (struct sockaddr *) &sockname, namelen, flags)) < 0)
    {
     ...
     * of calling something like TRANS_XSERV(SocketUNIXGetAddr).
     */
...
static int
TRANS_XSERV(SocketUNIXResetListener) (XtransConnInfo ciptr)
...
static XtransConnInfo
TRANS_XSERV(SocketINETAccept) (XtransConnInfo ciptr, int *status)

{
	...
	*status = TRANS_ACCEPT_FAILED;
    ...
    if (TRANS_XSERV(SocketINETGetAddr) (newciptr) < 0)
    {
	prmsg (1,
	...
	*status = TRANS_ACCEPT_MISC_ERROR;
    ...
    if (TRANS_XSERV(SocketINETGetPeerAddr) (newciptr) < 0)
    {
	prmsg (1,
...
#ifdef UNIXCONN
static XtransConnInfo
TRANS_XSERV(SocketUNIXAccept) (XtransConnInfo ciptr, int *status)
...
static int
TRANS_XSERV(SocketINETConnect) (XtransConnInfo ciptr,
                          const char *host, const char *port)

{
    ...
    {
        hostnamebuf[0] = '\0';
        (void) TRANS_XSERV(GetHostname) (hostnamebuf, sizeof hostnamebuf);
        host = hostnamebuf;
    }
	...
	while (socketaddr == NULL) {
		...
		if (Sockettrans2devtab[ciptr->index].family == AF_INET6) {
			 ...
			 */
			TRANS_XSERV(SocketINETClose)(ciptr);
			newciptr = TRANS_XSERV(SocketOpenCOTSClientBase)(
...
#define INADDR_NONE ((in_addr_t) 0xffffffff)
    ...
    if (connect (ciptr->fd, socketaddr, socketaddrlen ) < 0)
    {
        ...
        else if (TRANS_XSERV(SocketINETGetPeerAddr) (ciptr) < 0)
        {
            prmsg (1,
...

{
	...
	return equiv;
         ...
         * by TRANS_XSERV(GetHostname)), then the two hostnames are equivalent,
...
static int
TRANS_XSERV(SocketUNIXConnect) (XtransConnInfo ciptr,
                          const char *host, const char *port)
...
static int
TRANS_XSERV(SocketBytesReadable) (XtransConnInfo ciptr, BytesReadable_t *pend)
...
static int
TRANS_XSERV(SocketRecvFd) (XtransConnInfo ciptr)
{
...
static int
TRANS_XSERV(SocketSendFd) (XtransConnInfo ciptr, int fd, int do_close)
{
...
static int
TRANS_XSERV(SocketRecvFdInvalid)(XtransConnInfo ciptr)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_XSERV(SocketSendFdInvalid)(XtransConnInfo ciptr, int fd, int do_close)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_XSERV(SocketRead) (XtransConnInfo ciptr, char *buf, int size)
...
static int
TRANS_XSERV(SocketReadv) (XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_XSERV(SocketWritev) (XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_XSERV(SocketWrite) (XtransConnInfo ciptr, char *buf, int size)

{
    ...
    if (ciptr->send_fds)
    {
        ...
        iov.iov_base = buf;
        iov.iov_len = size;
        return TRANS_XSERV(SocketWritev)(ciptr, &iov, 1);
    }
#endif /* XTRANS_SEND_FDS */
...
static int
TRANS_XSERV(SocketDisconnect) (XtransConnInfo ciptr)
...
#ifdef TCPCONN
static int
TRANS_XSERV(SocketINETClose) (XtransConnInfo ciptr)
...
#ifdef UNIXCONN
static int
TRANS_XSERV(SocketUNIXClose) (XtransConnInfo ciptr)
{
    /*
...
static int
TRANS_XSERV(SocketUNIXCloseForCloning) (XtransConnInfo ciptr)
