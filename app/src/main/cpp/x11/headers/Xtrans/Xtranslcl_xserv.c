...
static int TRANS_XSERV(LocalClose)(XtransConnInfo ciptr);
...
static int
TRANS_XSERV(OpenFail)(XtransConnInfo ciptr _X_UNUSED, const char *port _X_UNUSED)
...
static int
TRANS_XSERV(ReopenFail)(XtransConnInfo ciptr _X_UNUSED, int fd _X_UNUSED,
...
#if XTRANS_SEND_FDS
static int
TRANS_XSERV(LocalRecvFdInvalid)(XtransConnInfo ciptr)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_XSERV(LocalSendFdInvalid)(XtransConnInfo ciptr, int fd, int do_close)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_XSERV(FillAddrInfo)(XtransConnInfo ciptr,
...
static int
TRANS_XSERV(NAMEDOpenClient)(XtransConnInfo ciptr, const char *port)

{
    ...
    if (TRANS_XSERV(FillAddrInfo) (ciptr, server_path, server_path) == 0)
    {
...
#ifdef NAMEDNODENAME
static int
TRANS_XSERV(NAMEDOpenPipe)(const char *server_path)
{
    int			fd, pipefd[2];
    struct stat		sbuf;
    int			mode;
...
static int
TRANS_XSERV(NAMEDOpenServer)(XtransConnInfo ciptr, const char *port)
{
#ifdef NAMEDNODENAME
    int			fd;
    char		server_path[64];
    ...
    fd = TRANS_XSERV(NAMEDOpenPipe)(server_path);
    if (fd < 0) {
	return -1;
    }
...
static int
TRANS_XSERV(NAMEDResetListener) (XtransConnInfo ciptr)

{
  ...
  if (ciptr->fd != -1) {
	...
	(statb.st_mode & S_IFMT) != S_IFIFO) {
      ...
      TRANS_XSERV(LocalClose)(ciptr);
      ciptr->fd = TRANS_XSERV(NAMEDOpenPipe)(sockname->sun_path);
      if (ciptr->fd >= 0)
...
static int
TRANS_XSERV(NAMEDAccept)(XtransConnInfo ciptr, XtransConnInfo newciptr, int *status)
...
static int
TRANS_XSERV(NAMEDReopenServer)(XtransConnInfo ciptr, int fd _X_UNUSED, const char *port)
...
static LOCALtrans2dev LOCALtrans2devtab[] = {
        {"",
#ifdef TRANS_CLIENT
         TRANS_XSERV(NAMEDOpenClient),
#endif /* TRANS_CLIENT */
#ifdef TRANS_SERVER
                TRANS_XSERV(NAMEDOpenServer),
#endif /* TRANS_SERVER */
#ifdef TRANS_CLIENT
         TRANS_XSERV(OpenFail),
#endif /* TRANS_CLIENT */
#ifdef TRANS_SERVER
                TRANS_XSERV(OpenFail),
#endif /* TRANS_SERVER */
#ifdef TRANS_REOPEN
                TRANS_XSERV(NAMEDReopenServer),
     TRANS_XSERV(ReopenFail),
#endif
#ifdef TRANS_SERVER
                TRANS_XSERV(NAMEDResetListener),
     TRANS_XSERV(NAMEDAccept)
#endif /* TRANS_SERVER */
        },
...
static void
TRANS_XSERV(LocalInitTransports)(const char *protocol)
...
static void
TRANS_XSERV(LocalEndTransports)(void)
...
TRANS_XSERV(LocalGetNextTransport)(void)
...
TRANS_XSERV(LocalOpenClient)(int type, const char *protocol,
...

{
    ...
    TRANS_XSERV(LocalInitTransports)(protocol);

    index = 0;
    for(transptr=TRANS_XSERV(LocalGetNextTransport)();
        transptr!=NULL;transptr=TRANS_XSERV(LocalGetNextTransport)(), index++)
    {
        switch( type )
        {
    ...
    TRANS_XSERV(LocalEndTransports)();
...
static XtransConnInfo
TRANS_XSERV(LocalOpenServer)(int type, const char *protocol,
...
static XtransConnInfo
TRANS_XSERV(LocalReopenServer)(int type, int index, int fd, const char *port)
...
static XtransConnInfo
TRANS_XSERV(LocalOpenCOTSClient)(Xtransport *thistrans _X_UNUSED, const char *protocol,
                           const char *host, const char *port)

{
    prmsg(2,"LocalOpenCOTSClient(%s,%s,%s)\n",protocol,host,port);

    return TRANS_XSERV(LocalOpenClient)(XTRANS_OPEN_COTS_CLIENT, protocol, host, port);
}
...
static XtransConnInfo
TRANS_XSERV(LocalOpenCOTSServer)(Xtransport *thistrans, const char *protocol,
			   const char *host, const char *port)

{
    ...
    TRANS_XSERV(LocalInitTransports)("local");
    typetocheck = workingXLOCAL;
    ...
    return TRANS_XSERV(LocalOpenServer)(XTRANS_OPEN_COTS_SERVER, protocol, host, port);
}
...
static XtransConnInfo
TRANS_XSERV(LocalReopenCOTSServer)(Xtransport *thistrans, int fd, const char *port)

{
    ...
    return TRANS_XSERV(LocalReopenServer)(XTRANS_OPEN_COTS_SERVER,
	index, fd, port);
}
...
static int
TRANS_XSERV(LocalSetOption)(XtransConnInfo ciptr, int option, int arg)
...
static int
TRANS_XSERV(LocalCreateListener)(XtransConnInfo ciptr, const char *port,
...
static int
TRANS_XSERV(LocalResetListener)(XtransConnInfo ciptr)
...
static XtransConnInfo
TRANS_XSERV(LocalAccept)(XtransConnInfo ciptr, int *status)
...
static int
TRANS_XSERV(LocalConnect)(XtransConnInfo ciptr,
                    const char *host _X_UNUSED, const char *port)
...
static int
TRANS_XSERV(LocalBytesReadable)(XtransConnInfo ciptr, BytesReadable_t *pend )
...
static int
TRANS_XSERV(LocalRead)(XtransConnInfo ciptr, char *buf, int size)
...
static int
TRANS_XSERV(LocalWrite)(XtransConnInfo ciptr, char *buf, int size)
...
static int
TRANS_XSERV(LocalReadv)(XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_XSERV(LocalWritev)(XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_XSERV(LocalDisconnect)(XtransConnInfo ciptr)
...
static int
TRANS_XSERV(LocalClose)(XtransConnInfo ciptr)
...
static int
TRANS_XSERV(LocalCloseForCloning)(XtransConnInfo ciptr)
