...
#ifdef TCPCONN
static int TRANS_ICE(SocketINETClose) (XtransConnInfo ciptr);
#endif
...
static int
TRANS_ICE(SocketSelectFamily) (int first, const char *family)
...
static int
TRANS_ICE(SocketINETGetAddr) (XtransConnInfo ciptr)
...
static int
TRANS_ICE(SocketINETGetPeerAddr) (XtransConnInfo ciptr)
...
TRANS_ICE(SocketOpen) (int i, int type)
...
static XtransConnInfo
TRANS_ICE(SocketReopen) (int i _X_UNUSED, int type, int fd, const char *port)
...
static XtransConnInfo
TRANS_ICE(SocketOpenCOTSClientBase) (const char *transname, const char *protocol,
                                 const char *host, const char *port, int previndex)
{
    ...
    while ((i = TRANS_ICE(SocketSelectFamily) (i, transname)) >= 0) {
        if ((ciptr = TRANS_ICE(SocketOpen) (
...
static XtransConnInfo
TRANS_ICE(SocketOpenCOTSClient) (Xtransport *thistrans, const char *protocol,
                             const char *host, const char *port)
{
    return TRANS_ICE(SocketOpenCOTSClientBase)(
...
static XtransConnInfo
TRANS_ICE(SocketOpenCOTSServer) (Xtransport *thistrans, const char *protocol,
			     const char *host, const char *port)

{
    ...
    while ((i = TRANS_ICE(SocketSelectFamily) (i, thistrans->TransName)) >= 0) {
	if ((ciptr = TRANS_ICE(SocketOpen) (
		 i, Sockettrans2devtab[i].devcotsname)) != NULL)
	    break;
    }
...
static XtransConnInfo
TRANS_ICE(SocketReopenCOTSServer) (Xtransport *thistrans, int fd, const char *port)

{
    ...
    while ((i = TRANS_ICE(SocketSelectFamily) (i, thistrans->TransName)) >= 0) {
	if ((ciptr = TRANS_ICE(SocketReopen) (
...
static int
TRANS_ICE(SocketSetOption) (XtransConnInfo ciptr, int option, int arg)
...
static int
TRANS_ICE(SocketCreateListener) (XtransConnInfo ciptr,
...
#ifdef TCPCONN
static int
TRANS_ICE(SocketINETCreateListener) (XtransConnInfo ciptr, const char *port,
                                 unsigned int flags)

{
    ...
    if ((status = TRANS_ICE(SocketCreateListener) (ciptr,
    ...
    if (TRANS_ICE(SocketINETGetAddr) (ciptr) < 0)
    {
	prmsg (1,
...
static int
TRANS_ICE(SocketUNIXCreateListener) (XtransConnInfo ciptr, const char *port,
				 unsigned int flags)

{
    ...
    if ((status = TRANS_ICE(SocketCreateListener) (ciptr, (struct sockaddr *) &sockname, namelen, flags)) < 0)
    {
     ...
     * of calling something like TRANS_ICE(SocketUNIXGetAddr).
     */
...
static int
TRANS_ICE(SocketUNIXResetListener) (XtransConnInfo ciptr)
...
static XtransConnInfo
TRANS_ICE(SocketINETAccept) (XtransConnInfo ciptr, int *status)

{
	...
	*status = TRANS_ACCEPT_FAILED;
    ...
    if (TRANS_ICE(SocketINETGetAddr) (newciptr) < 0)
    {
	prmsg (1,
	...
	*status = TRANS_ACCEPT_MISC_ERROR;
    ...
    if (TRANS_ICE(SocketINETGetPeerAddr) (newciptr) < 0)
    {
	prmsg (1,
...
#ifdef UNIXCONN
static XtransConnInfo
TRANS_ICE(SocketUNIXAccept) (XtransConnInfo ciptr, int *status)
...
static int
TRANS_ICE(SocketINETConnect) (XtransConnInfo ciptr,
                          const char *host, const char *port)

{
    ...
    {
        hostnamebuf[0] = '\0';
        (void) TRANS_ICE(GetHostname) (hostnamebuf, sizeof hostnamebuf);
        host = hostnamebuf;
    }
	...
	while (socketaddr == NULL) {
		...
		if (Sockettrans2devtab[ciptr->index].family == AF_INET6) {
			 ...
			 */
			TRANS_ICE(SocketINETClose)(ciptr);
			newciptr = TRANS_ICE(SocketOpenCOTSClientBase)(
...
#define INADDR_NONE ((in_addr_t) 0xffffffff)
    ...
    if (connect (ciptr->fd, socketaddr, socketaddrlen ) < 0)
    {
        ...
        else if (TRANS_ICE(SocketINETGetPeerAddr) (ciptr) < 0)
        {
            prmsg (1,
...

{
	...
	return equiv;
         ...
         * by TRANS_ICE(GetHostname)), then the two hostnames are equivalent,
...
static int
TRANS_ICE(SocketUNIXConnect) (XtransConnInfo ciptr,
                          const char *host, const char *port)
...
static int
TRANS_ICE(SocketBytesReadable) (XtransConnInfo ciptr, BytesReadable_t *pend)
...
static int
TRANS_ICE(SocketRecvFd) (XtransConnInfo ciptr)
{
...
static int
TRANS_ICE(SocketSendFd) (XtransConnInfo ciptr, int fd, int do_close)
{
...
static int
TRANS_ICE(SocketRecvFdInvalid)(XtransConnInfo ciptr)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_ICE(SocketSendFdInvalid)(XtransConnInfo ciptr, int fd, int do_close)
{
    errno = EINVAL;
    return -1;
}
...
static int
TRANS_ICE(SocketRead) (XtransConnInfo ciptr, char *buf, int size)
...
static int
TRANS_ICE(SocketReadv) (XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_ICE(SocketWritev) (XtransConnInfo ciptr, struct iovec *buf, int size)
...
static int
TRANS_ICE(SocketWrite) (XtransConnInfo ciptr, char *buf, int size)

{
    ...
    if (ciptr->send_fds)
    {
        ...
        iov.iov_base = buf;
        iov.iov_len = size;
        return TRANS_ICE(SocketWritev)(ciptr, &iov, 1);
    }
#endif /* XTRANS_SEND_FDS */
...
static int
TRANS_ICE(SocketDisconnect) (XtransConnInfo ciptr)
...
#ifdef TCPCONN
static int
TRANS_ICE(SocketINETClose) (XtransConnInfo ciptr)
...
#ifdef UNIXCONN
static int
TRANS_ICE(SocketUNIXClose) (XtransConnInfo ciptr)
{
    /*
...
static int
TRANS_ICE(SocketUNIXCloseForCloning) (XtransConnInfo ciptr)
