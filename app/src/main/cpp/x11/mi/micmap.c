               ...
               unsigned short *pblue, VisualPtr pVisual)
{
    ...
    if ((pVisual->_class | DynamicClass) == GrayScale) {
...
miInitializeColormap(ColormapPtr pmap)
{
    ...
    else if (pVisual->_class == StaticColor) {
        unsigned limr, limg, limb;
    ...
    else if (pVisual->_class == StaticGray) {
        for (i = 0; i <= maxent; i++) {
            /* rescale to [0..65535] then rgb bits */
...
miCreateDefColormap(ScreenPtr pScreen)
{
    ...
    if (pScreen->rootDepth == 1 || (pVisual->_class & DynamicClass))
        alloctype = AllocNone;
    else
        alloctype = AllocAll;
        ...
        *prefp = visuals->preferredCVC;
        ...
        for (i = 0; i < NUM_PRIORITY; i++) {
            if (!(vtype & (1 << miVisualPriority[i])))
                continue;
            visual->_class = miVisualPriority[i];
    ...
    for (i = first_depth; i < ndepth; i++) {
        ...
        for (j = 0; j < depth[i].numVids; j++) {
            ...
            if (prefColorVisualClass < 0 ||
                visual[k]._class == prefColorVisualClass)
                break;
        }
