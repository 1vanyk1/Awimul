...
#include "../headers/xatom.h"          /* must come after server includes */
...
CreateRootWindow(ScreenPtr pScreen)
{
    ...
    pWin->drawable._class = InputOutput;
    pWin->optional->visual = pScreen->rootVisual;
             ...
             int depth, ClientPtr client, VisualID visual, int *error)
{
    ...
    if ((class != InputOnly) && (pParent->drawable._class == InputOnly)) {
        *error = BadMatch;
        return NullWindow;
    }
...
ChangeWindowAttributes(WindowPtr pWin, Mask vmask, XID *vlist, ClientPtr client)
{
    ...
    if ((pWin->drawable._class == InputOnly) &&
...
#define REBORDER_WIN   3
    ...
    if ((pWin->drawable._class == InputOnly) && (mask & CWBorderWidth))
        return BadMatch;
...
RealizeTree(WindowPtr pWin)
{
    ...
    while (1) {
        if (pChild->mapped) {
            ...
            pChild->viewable = (pChild->drawable._class == InputOutput);
            (*Realize) (pChild);
            if (pChild->firstChild) {
                pChild = pChild->firstChild;
                continue;
...
MapWindow(WindowPtr pWin, ClientPtr client)
{
    ...
    if ((pParent = pWin->parent)) {
        ...
        pWin->viewable = pWin->drawable._class == InputOutput;
        /* We SHOULD check for an error value here XXX */
