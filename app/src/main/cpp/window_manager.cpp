...
void WindowManager::Run() {
    ...
    //   exit gracefully if another window manager is already running.
    {
        ...
        if (wm_detected_) {
            ALOGE("Detected another window manager on display %s", XDisplayString(display_));
            return;
        }
    }
    //   b. Set error handler.
...
//    CHECK_EQ(returned_root, root_);
    ...
    for (;;) {
        ...
        ALOGI("Received event: %s", ToString(e).c_str());
...
//    CHECK(!clients_.count(w));
    ...
    ALOGI("Framed window %s [%s]", ToString(w).c_str(), ToString(frame).c_str());
}
...
//    CHECK(clients_.count(w));
    ...
    ALOGI("Unframed window %s [%s]", ToString(w).c_str(), ToString(frame).c_str());
}
...
void WindowManager::OnUnmapNotify(const XUnmapEvent& e) {
    ...
    if (!clients_.count(e.window)) {
        ALOGI("Ignore UnmapNotify for non-client window %s", ToString(e.window).c_str());
        return;
    }
    ...
    if (e.event == root_) {
        ALOGI("Ignore UnmapNotify for reparented pre-existing window %s", ToString(e.window).c_str());
        return;
    }
...
void WindowManager::OnConfigureRequest(const XConfigureRequestEvent& e) {
    ...
    if (clients_.count(e.window)) {
        ...
        ALOGI("Resize [%s] to %s", ToString(frame).c_str(), ToString(Size<int>(e.width, e.height)).c_str());
    }
    XConfigureWindow(display_, e.window, e.value_mask, &changes);
    ALOGI("Resize %s to %s", ToString(e.window).c_str(), ToString(Size<int>(e.width, e.height)).c_str());
}
...
//    CHECK(clients_.count(e.window));
    ...
    XGetGeometry(display_, frame, &returned_root, &x, &y, &width, &height, &border_width, &depth);
    ...
    if ((e.state & Mod1Mask) &&
        ...
        if (XGetWMProtocols(display_,
            ...
            (::std::find(supported_protocols,
                         ...
                         WM_DELETE_WINDOW) !=
             supported_protocols + num_supported_protocols)) {
            ALOGI("Gracefully deleting window %s", ToString(e.window).c_str());
            // 1. Construct message.
            XEvent msg;
            ...
            ALOGI("Killing window %s", ToString(e.window).c_str());
...
int WindowManager::OnXError(Display* display, XErrorEvent* e) {
    ...
    ALOGE("Received X error:\n"
          "    Request: %d - %s\n"
          "    Error code: %d - %s\n"
          "    Resource ID: %s",
          int(e->request_code), XRequestCodeToString(e->request_code).c_str(),
          int(e->error_code), error_text, ToString(e->resourceid).c_str());
